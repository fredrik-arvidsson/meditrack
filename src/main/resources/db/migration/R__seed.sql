...
-- ---------- LAGERPOSTER ----------
-- Saldon för alla sju läkemedlen. Paracetamol och Salbutamol är
...
VALUES
    ...
    (6, 1, 3, 200, 50, 0, NOW(6), 1, NOW(6), 1),  -- Amoxicillin: ok
    (7, 1, 6,  80, 20, 0, NOW(6), 1, NOW(6), 1);  -- Natriumklorid: ok

-- ---------- BESTÄLLNINGAR ----------
-- En i SENT (skickad, väntar på bekräftelse) och en levererad.
-- Den skickade ger demon en order där "Bekräfta" är nästa lagliga
-- action — sjuksköterska Sara (3) skickade den, en apotekare
-- bekräftar (separation of duties, HSLF-FS 2017:37). Den levererade
...
VALUES
    (1, 1, 'ORD-DEMO0001', 'SENT',
     DATE_SUB(NOW(6), INTERVAL 1 DAY), 3,
     NULL, NULL,
     NULL, NULL,
     'Skickad av sjuksköterska — väntar på bekräftelse från apotek', 0,
     DATE_SUB(NOW(6), INTERVAL 2 DAY), 3, DATE_SUB(NOW(6), INTERVAL 1 DAY), 3),
...
VALUES
    -- Skickad (order 1): två rader, väntar på bekräftelse
    (1, 1, 6, 50, NULL, DATE_SUB(NOW(6), INTERVAL 2 DAY), 3, DATE_SUB(NOW(6), INTERVAL 2 DAY), 3),
    (2, 1, 3, 30, NULL, DATE_SUB(NOW(6), INTERVAL 2 DAY), 3, DATE_SUB(NOW(6), INTERVAL 2 DAY), 3),
    -- Levererad (order 2): en rad, Paracetamol
    (3, 2, 1, 100, 'Stor påfyllnad', DATE_SUB(NOW(6), INTERVAL 6 DAY), 3, NOW(6), 2);

-- ---------- LAGERRÖRELSER ----------
-- Startsaldon (INITIAL) för lagerposterna, plus leveransen
...
VALUES
    ...
    (6, 1, 1, 1, 100, 100, 'DELIVERY',  2,    'Leverans av order ORD-DEMO0002',
     DATE_SUB(NOW(6), INTERVAL 1 DAY), 2),
    (7, 1, 6, 3, 200, 200, 'INITIAL',   NULL, 'Startsaldo',
     DATE_SUB(NOW(6), INTERVAL 30 DAY), 1),
    (8, 1, 7, 6, 80,  80,  'INITIAL',   NULL, 'Startsaldo',
     DATE_SUB(NOW(6), INTERVAL 30 DAY), 1);
