...
**Datum:** 2026-05-27
**Källa:** Regelverk-informerad design (GDPR, PDL, NIS2, HSLF-FS 2017:37, SOSFS 2011:9, MDR gränsfall) + lärdomar från Pastoralen-inventeringen.
...
Matchar PDL:s isoleringskrav. Pastoralens läxa: bättre att isolera från dag 1 än retrofitta.
...
`UNIQUE (care_unit_id, email)`: e-post unik **per vårdenhet**, inte globalt. Förenklar soft-delete-flödet. Motivera i README.
...
`version`-kolumnen för Hibernates `@Version` — optimistisk låsning vid concurrent updates. Faraz frågar uttryckligen om detta på intervjun.
...
✅ **Optimistic locking via `version`** på Order och StockItem (concurrency — Faraz-fråga)
...
Allt i **en** migration `V1__init.sql`. Seed-data som `R__seed.sql` (repeatable) — Pastoralens läxa: håll seed separat från schema.
